...
func main() {
	...
	for true {
		...
		if len(userinput) > 1 {
			if userinput == pendu.mot_a_trouver {
				pendu.mot_brouillé = pendu.mot_a_trouver
			} else {
				pendu.nombre_essais = pendu.nombre_essais + 2
				tentative = tentative - 2
			}
		}
		if pendu.nombre_essais >= 10 {
			fmt.Println("Vous avez perdu !")
			fmt.Println("Le mot à trouver était : ", pendu.mot_a_trouver)
			break
		}
		if pendu.mot_a_trouver == pendu.mot_brouillé {
			fmt.Println("Vous avez gagné \nLe mot était : ", pendu.mot_a_trouver)
			break
		}
	}
}